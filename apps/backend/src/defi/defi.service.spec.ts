...
describe('DefiService', () => {
  ...
  beforeAll(async () => {
    const chainStub: Partial<ChainService> = {
      ...
      simulateContractCallReal: async (_input?: any) => ({
        ok: true,
        estimatedFee: 0.0001,
      }),
      submitTxReal: async (_input?: any) => ({
        ok: true,
        txHash: 'TX_TEST',
        error: undefined,
      }),
  ...
  it('should support loan policy decisions deterministically', async () => {
    const approved = await service.loanByScore({
      userScore: 700,
      amount: 10_000,
    });
    expect(approved.ok).toBe(true);

    const denied = await service.loanByScore({
      userScore: 500,
      amount: 10_000,
    });
  ...
  it('should support dry-run for stake, unstake and LP ops', async () => {
    const stake = await service.stake({
      poolId: 'POOL1',
      amount: 10,
      dryRun: true,
    });
    ...
    const unstake = await service.unstake({
      poolId: 'POOL1',
      amount: 5,
      dryRun: true,
    });
    ...
    const addLp = await service.addLiquidity({
      poolId: 'POOL1',
      amounts: [1, 2],
      dryRun: true,
    });
    ...
    const remLp = await service.removeLiquidity({
      poolId: 'POOL1',
      share: 1,
      dryRun: true,
    });
  ...
  it('should apply flash loan guard policy', async () => {
    const allowed = await service.flashLoanGuard({
      txPreview: {},
      maxAmount: 50_000,
      riskBps: 500,
    });
    expect(allowed.ok).toBe(true);
    const blocked = await service.flashLoanGuard({
      txPreview: {},
      maxAmount: 200_000,
      riskBps: 900,
    });
  ...
  describe('Staking Operations', () => {
    ...
    it('should handle different pool IDs', async () => {
      const pool1 = await service.stake({
        poolId: 'POOL1',
        amount: 10,
        dryRun: true,
      });
      const pool2 = await service.stake({
        poolId: 'POOL2',
        amount: 20,
        dryRun: true,
      });
    ...
    it('should handle varying stake amounts', async () => {
      const small = await service.stake({
        poolId: 'POOL1',
        amount: 1,
        dryRun: true,
      });
      const large = await service.stake({
        poolId: 'POOL1',
        amount: 1000000,
        dryRun: true,
      });
  ...
  describe('Unstaking Operations', () => {
    ...
    it('should handle different unstake amounts', async () => {
      const partial = await service.unstake({
        poolId: 'POOL1',
        amount: 10,
        dryRun: true,
      });
      const full = await service.unstake({
        poolId: 'POOL1',
        amount: 100,
        dryRun: true,
      });
  ...
  describe('Liquidity Operations', () => {
    ...
    it('should remove liquidity with different share amounts', async () => {
      const small = await service.removeLiquidity({
        poolId: 'POOL1',
        share: 0.1,
        dryRun: true,
      });
      const large = await service.removeLiquidity({
        poolId: 'POOL1',
        share: 0.9,
        dryRun: true,
      });
  ...
  describe('Loan Scoring', () => {
    it('should approve loans for high credit scores', async () => {
      const result = await service.loanByScore({
        userScore: 800,
        amount: 50000,
      });
      expect(result.ok).toBe(true);
    ...
    it('should consider loan amount in approval', async () => {
      const smallLoan = await service.loanByScore({
        userScore: 700,
        amount: 1000,
      });
      const largeLoan = await service.loanByScore({
        userScore: 700,
        amount: 100000,
      });
