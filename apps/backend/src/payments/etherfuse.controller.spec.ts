...
describe('EtherfuseController', () => {
  ...
  it('should create etherfuse quote', async () => {
    ...
    etherfuseService.createQuote.mockResolvedValue({
      ok: true,
      quote: { id: 'quote-1' },
    });

    await expect(controller.createQuote(dto)).resolves.toEqual({
      ok: true,
      quote: { id: 'quote-1' },
    });
  ...
  it('should create etherfuse order', async () => {
    ...
    etherfuseService.createOrder.mockResolvedValue({
      ok: true,
      order: { id: 'order-1' },
    });
