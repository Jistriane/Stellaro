...
const createMocks = () => {
  const oracles = { getPrice: jest.fn() } as unknown as jest.Mocked<OraclesService>;
  const notifications = { send: jest.fn() } as unknown as jest.Mocked<NotificationsService>;
  const compliance = {
    kycCheck: jest.fn(),
...
describe('AutomationService', () => {
  ...
  it('riskPipeline returns price and triggers notification when configured', async () => {
    ...
    const result = await service.riskPipeline({ asset: 'USD/BRL', thresholdBps: 50, notifyTo: 'user@test' });

    expect(result.ok).toBe(true);
    expect(result.price.value).toBe(5.1);
    expect(oracles.getPrice).toHaveBeenCalledWith('USD', 'BRL');
    expect(notifications.send).toHaveBeenCalledWith('email', 'user@test', 'Risk Pipeline', 'USD/BRL=5.1');
  });
  ...
  it('onboardingPipeline returns KYC result and optionally notifies', async () => {
    ...
    const result = await service.onboardingPipeline({ document: '123', name: 'Alice', notifyTo: 'user@test' });

    expect(result.ok).toBe(true);
    expect(result.kyc.level).toBe('APPROVED');
    expect(compliance.kycCheck).toHaveBeenCalledWith('123', 'Alice');
    expect(notifications.send).toHaveBeenCalledWith('email', 'user@test', 'Onboarding', 'KYC=APPROVED');
  });
  ...
  it('compliancePipeline returns AML result and notification when requested', async () => {
    ...
    const result = await service.compliancePipeline({ address: 'GABC', notifyTo: 'user@test' });

    expect(result.ok).toBe(false);
    expect(result.aml.flagged).toBe(true);
    expect(compliance.amlScreening).toHaveBeenCalledWith('GABC');
    expect(notifications.send).toHaveBeenCalledWith('email', 'user@test', 'Compliance', 'AML flagged=true');
  });

  it('creditPipeline computes clamped score and notifies', async () => {
    const result = await service.creditPipeline({ userId: 'u1', income: 5000, notifyTo: 'user@test' });

    expect(result.ok).toBe(true);
    expect(result.score).toBe(500);
    expect(notifications.send).toHaveBeenCalledWith('email', 'user@test', 'Credit', 'Score=500');
  });

  it('creditPipeline clamps score to 1000 when income is high', async () => {
    const result = await service.creditPipeline({ userId: 'u1', income: 20000 });
  ...
  it('reportsPipeline returns localized content and can notify', async () => {
    const resultPt = await service.reportsPipeline({ lang: 'pt', notifyTo: 'user@test' });
    ...
    expect(notifications.send).toHaveBeenCalledWith('email', 'user@test', 'Relatório', expect.stringContaining('Relatório consolidado'));
  });
});
